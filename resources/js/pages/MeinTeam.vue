    ...
    <div class="space-y-8 p-6">
        ...
        <section v-for="group in memberGroups" :key="group.title" class="space-y-5">
            ...
            <div v-if="group.members.length" class="space-y-3">
                <article
                    ...
                    :key="member.id"
                    class="grid gap-4 rounded-lg border p-4 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center"
                >
                        ...
                        <TooltipProvider v-if="member.role !== 'owner' && permissions.canRemoveMember">
                            <Tooltip>
                                <TooltipTrigger as-child>
                                    <Button variant="ghost" size="sm" @click="confirmRemoveMember(member)">
                                        <X class="h-4 w-4" />
                                    </Button>
                                </TooltipTrigger>
                                <TooltipContent>Mitglied entfernen</TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
                    </div>
                </article>
        ...
        <section v-if="permissions.canCreateInvitation && invitations.length" class="space-y-5">
            <Heading
                ...
                description="Diese Accounts müssen ihre Einladung noch annehmen."
            />
            <div class="space-y-3">
                <article
                ...
                >
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">{{ invitation.role_label }}</p>
                    </div>
                    <TooltipProvider v-if="permissions.canCancelInvitation">
                        <Tooltip>
                            <TooltipTrigger as-child>
                                <Button variant="ghost" size="sm" @click="confirmCancelInvitation(invitation)">
                                    <X class="h-4 w-4" />
                                </Button>
                            </TooltipTrigger>
